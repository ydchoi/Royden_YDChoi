...
Hence, we have proven that if $m(E) < \infty$, and $\epsilon > 0$,
then $E$ is the disjoint union of a finite number of measurable sets, each of which has measure
at most $\epsilon$ for the $E$ bounded case. \\

\smallskip

Now assume that $E$ is unbounded. Consider the following disjoint collection of sets:
\eQb
\{ [M-1,M) \cap E \}_{M = -\infty}^{\infty}. 
\eQe
Notice that $E = \cup_{M = -\infty}^{\infty} [M-1,M) \cap E$. Furthermore, from the same reasoning
as above, each set in the collection is measurable. By the countable additivity, we have
\eQb
m(E) &=& \sum_{M = -\infty}^{\infty} m([M-1, M) \cap E).
\eQe
