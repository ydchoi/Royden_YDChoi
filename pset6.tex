...
on the RHS, we have
\eQb
\text{dist}(x,E) &\leq& d(x,y) + \text{dist}(y,E).
\eQe
Hence, it follows that 
\eQb
\text{dist}(x,E) - \text{dist}(y,E) &\leq& d(x,y).
\eQe
By symmetry, we also obtain that
\eQb
\text{dist}(y,E) - \text{dist}(x,E) &\leq& d(x,y).
\eQe
Hence, it follows that
\eQb
| \text{dist}(x,E) - \text{dist}(y,E)| &\leq& d(x,y).
...
\eQb
\underset{n \to \infty}{\lim} \sigma(f(u_n), f(v_n)) \to 0,
\eQe
as desired. Conversely, assume that $\lim_{n \to \infty}
\sigma(f(u_n),f(v_n)) \to 0$. Fix $\epsilon > 0$. Then, there 
exists an index $N$ such that $\sigma(f(u_n), f(v_n)) < \epsilon$,
for $n \geq N$. 
\end{solution}
...
$O = O_X \times O_Y$, where $O_X$ and $O_Y$ open subsets of O. 
